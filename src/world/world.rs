...
impl<'a, R: Rng + 'a> Model<'a> for World<R> {
    ...
    fn step(&mut self, cmd: Option<Self::Cmd>) -> ::std::result::Result<Self::Update, Self::Error> {
        if let Some(dir) = cmd {
